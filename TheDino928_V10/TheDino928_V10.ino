...
void setup() 
{
  ...
  tick.every(50, Timer100msHandler);//setup 50ms timer to check out the event flag to service
  PID_Event=tick.every(5, PD_Control);// setup PID control Timer
...
void OneSecTimerHandler() //-- update per sec -- 
{
        ...
        float SpeedCMMin;
        float Battery_voltage,BatteryRatio ; // Var for DS18B20 Temp reading

//------------- Read Battery voltage level from A0 ------
          Battery_voltage = analogRead(A0) ; // Full battery is around 4.18 for one 18650 Battery
          BatteryRatio = map(Battery_voltage,MinBatteryLevel,FullBatteryLevel,0,100) ; // map to 0 ~ 100 %  revised 4/12/2013
          if( BatteryRatio < 0 )  // If Battery is less than 5% then warning a tone
            tone(Buzzer,262,20) ; // Enable Battery Low warning tone(Freq : 262 Hz,Do tone) for 100ms
...
//-------- Add DS18B20 Temp reading and transmi to Smart devices to show it -----
        ...
        Serial.println(BatteryRatio,0); // Transmit Battery Level to smart deivces
...
volatile int BackwardTimer100ms     ; //While detect drop or distance less than some threshold then make car to Backward for some time period 
//volatile byte EventFlag = B00000000 ;    // clear 8 bits boolean event flag 
...
{ 
  ...
  if(EventFlag & UltraSoundEvent ){  //---------- check the distance with UltraSound sensor ---- 
    if(!(--BackwardTimer100ms)) {
     BackwardTimer100ms = LeftRightTimePeriod ; //Start left or right turn timer
    ...
    if(USRange != 0 && USRange <= UltraSoundThreshold ){ // Check the distance of obstacle < 5cm then start some actions 
     ...
     DCMotor(LeftMotorSpeed,RightMotorSpeed,TurnOff); // Stop Motor 
     BackwardTimer100ms = BackwardTimePeriod ; // start timer to count
